...
func build_body() -> void:
	...
	column.add_child(_toggle("Антиблокировка (ABS)", Settings.assist_abs, &"assist_abs"))
	column.add_child(_toggle("Противобуксовочная", Settings.assist_traction, &"assist_traction"))
	column.add_child(_toggle("Стабилизация", Settings.assist_stability, &"assist_stability"))
	column.add_child(_slider("Чувствительность руля", Settings.steer_sensitivity, 0.4, 1.6, &"steer_sensitivity"))
	column.add_child(_slider("Мёртвая зона стика", Settings.gamepad_deadzone, 0.05, 0.4, &"gamepad_deadzone"))
	column.add_child(Widgets.separator())

	column.add_child(Widgets.label("Изображение", 16, UiTheme.SAND))
	column.add_child(_slider("Масштаб отрисовки", Settings.render_scale, 0.5, 1.5, &"render_scale"))
	column.add_child(_slider("Поле зрения", Settings.fov, 55.0, 100.0, &"fov"))
	column.add_child(_choice(
	...
	column.add_child(_toggle("Вертикальная синхронизация", Settings.vsync, &"vsync"))
	column.add_child(Widgets.separator())

	column.add_child(Widgets.label("Звук", 16, UiTheme.SAND))
	column.add_child(_slider("Общая громкость", Settings.volume_master, 0.0, 1.0, &"volume_master"))
	column.add_child(_slider("Двигатель", Settings.volume_engine, 0.0, 1.0, &"volume_engine"))
	column.add_child(_slider("Мир", Settings.volume_world, 0.0, 1.0, &"volume_world"))
	column.add_child(Widgets.separator())
...
func _toggle(title: String, value: bool, key: StringName) -> Control:
	var button := CheckBox.new()
	button.button_pressed = value
	button.toggled.connect(func(pressed: bool) -> void:
		Settings.set_option(&"game", key, pressed)
	)
...
func _slider(title: String, value: float, low: float, high: float, key: StringName) -> Control:
	...
	slider.value_changed.connect(func(new_value: float) -> void:
		...
		Settings.set_option(&"game", key, new_value)
	)
	var label := Widgets.label(title, 14, UiTheme.INK_DIM)
	label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
