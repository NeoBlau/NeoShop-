...
func mix(a: int, b: int) -> int:
	# Множитель обязан быть нечётным и различаться для соседних b. Вариант
	# `b | 1` выглядит короче, но склеивает 0 с 1, 2 с 3 и так далее: на доске
	# заказов это давало пары одинаковых предложений подряд.
	var x: int = a + MIX_A * (b * 2 + 1)
	x = (x ^ ushr(x, 30)) * MIX_B
	x = (x ^ ushr(x, 27)) * MIX_C
	return x ^ ushr(x, 31)
